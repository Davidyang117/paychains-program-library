...
if [[ -n $CI ]]; then
  ...
  if [[ -n $TRAVIS ]]; then
    ...
    # to how paychains-ci-gate is used to trigger PR builds rather than using the
    ...
    if [[ -n $BUILDKITE_TRIGGERED_FROM_BUILD_PIPELINE_SLUG ]]; then
      # The paychains-secondary pipeline should use the slug of the pipeline that
      # triggered it
    ...
    # the paychains-secondary pipeline
