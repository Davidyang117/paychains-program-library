...
# Script to deposit stakes and PAY into a stake pool, given the stake pool keyfile
...
create_user_stakes () {
  ...
  do
    ...
    paychains create-stake-account "$keys_dir/stake_$validator.json" "$pay_amount" --withdraw-authority "$authority" --stake-authority "$authority"
  done < "$validator_list"
}
...
delegate_user_stakes () {
  ...
  while read -r validator
  do
    paychains delegate-stake --force "$keys_dir/stake_$validator.json" "$validator" --stake-authority "$authority"
  done < "$validator_list"
}
...
deposit_stakes () {
  ...
  while read -r validator
  do
    stake=$(paychains-keygen pubkey "$keys_dir/stake_$validator.json")
...
create_user_stakes "$validator_list" "$pay_amount" $authority
...
echo "Depositing PAY into stake pool"
$spl_stake_pool deposit-sol "$stake_pool_pubkey" "$pay_amount"
