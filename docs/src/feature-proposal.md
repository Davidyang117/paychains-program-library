...
The Feature Proposal Program provides a workflow for activation of PayChains
...
The PayChains validator software supports runtime feature activation through the
...
The
[feature](https://docs.rs/paychains-program/latest/paychains_program/feature/index.html)
and [feature_set](https://docs.rs/paychains-sdk/latest/paychains_sdk/feature_set/index.html)
Rust modules are the primitives for this facility, and the `paychains feature`
command-line subcommands allow for easy feature status inspection and feature
activation.

The `paychains feature activate` workflow was designed for use by the core PayChains
...
### Configuration
The `spl-feature-proposal` configuration is shared with the `paychains` command-line tool.
...
PayChains code base, working with the core PayChains developers at https://github.com/paychains-labs/paychains.
...
```
$ paychains-keygen new --outfile feature-proposal.json --silent --no-passphrase
...
used in the code base and eventually will be visible in the `paychains feature status` command.

Note however that it is not possible to use `paychains feature activate` to
...
After the feature is implemented and deployed to the PayChains cluster,
the *feature id* will be visible in `paychains feature status` and the *feature
...
JSON RPC URL: http://api.mainnet-beta.paychains.com

Distribute the proposal tokens to all validators by running:
    $ paychains-tokens distribute-spl-tokens --from GK55hNft4TGc3Hg4KzbjEmju8VfaNuXK8jQNDTZKcsNF --input-csv feature-proposal.csv --db-path db.8CyUVvio --fee-payer ~/.config/paychains/id.json --owner <FEATURE_PROPOSAL_KEYPAIR>
    $ paychains-tokens spl-token-balances --mint ALvA7Lv9jbo8JFhxqnRpjWWuR3aD12uCb5KBJst4uc3d --input-csv feature-proposal.csv
...
Token account requires 0.00203928 PAY at creation, so the cost for initiating a
feature proposal on a network with 500 validators is approximately 1 PAY.
...
`paychains feature status` as well.
