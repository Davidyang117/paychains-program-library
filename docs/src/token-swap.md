...
A Uniswap-like exchange for the Token program on the PayChains blockchain,
...
Check out
[program repository](https://github.com/paychains-labs/paychains-program-library/tree/master/token-swap)
...
[JavaScript
bindings](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/js/src/index.ts)
...
[end-to-end tests](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/js/cli/token-swap-test.ts).

### Creating a new token swap pool

The creation of a pool showcases the account, instruction, and authorization
models on PayChains, which can be very different compared to other blockchains.
...
[program derived address](https://docs.paychains.com/developing/programming-model/calling-between-programs#program-derived-addresses)
...
Since PayChains programs require all accounts to be declared in the instruction,
...
that implements the
[CurveCalculator](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/calculator.rs)
...
curve](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/constant_product.rs)
...
The [constant price curve](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/constant_price.rs)
...
game creator wants to sell new "PAYGAME" tokens to be used in their
game, so they create a constant price swap of 2 USDC per PAYGAME, and supply all
of the PAYGAME tokens at swap creation. Users can go to the swap and purchase all
of the tokens they want and not worry about the market making PAYGAME tokens too
expensive.

### Stable (under construction)

The [stable curve](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/stable.rs)
...
The [offset curve](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/offset.rs)
...
For example, a decentralized betting application creator wants to sell new "PAYBET"
tokens on the market in exchange for USDC, and they believe each token is worth
at least 4 USDC.  They create a pool between PAYBET and USDC, funding
one side with 1,000 PAYBET, and the other side with 0 USDC, but an offset
of 4,000 USDC.

If a trader tries to buy PAYBET with 40 USDC, the invariant is calculated 
with the offset:

```
(PAYBET_total) * (USDC_total + USDC_offset) = invariant
1,000 * (0 + 4,000) = 4,000,000

(PAYBET_total - PAYBET_out) * (USDC_total + USDC_offset + USDC_in) = invariant
PAYBET_out = 9.901
```

The trader received 9.901 PAYBET for 40 USDC, so the price per PAYBET was
roughly 4.04, slightly higher than the minimum of 4 USDC per PAYBET.

Conversely, if a trader tries to buy USDC with PAYBET immediately after creation,
...
[curves](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/constant_product.rs)
and
[math](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/src/curve/math.rs)
...
tests](https://github.com/paychains-labs/paychains-program-library/blob/master/token-swap/program/fuzz/src/instructions.rs)
of the repo.
