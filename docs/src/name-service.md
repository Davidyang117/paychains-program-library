...
A SPL program for issuing and managing ownership of: domain names, PayChains Pubkeys, URLs, Twitter handles, ipfs cid's etc..

This program could be used for dns, pubkey etc lookups via a browser extension
for example, the goal is to create an easy way to identify PayChains public keys
with various links.
