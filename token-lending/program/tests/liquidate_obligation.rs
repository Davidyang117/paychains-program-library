...
async fn test_success() {
    ...
    // 100 PAY * 80% LTV -> 80 PAY * 20 USDC -> 1600 USDC borrow
    ...
    // 800 USDC / 20 USDC per PAY -> 40 PAY + 10% bonus -> 44 PAY
    const PAY_LIQUIDATION_AMOUNT_LAMPORTS: u64 = 44 * LAMPORTS_TO_PAY * INITIAL_COLLATERAL_RATIO;
    ...
    assert_eq!(
        ...
        initial_user_collateral_balance + PAY_LIQUIDATION_AMOUNT_LAMPORTS
    );
    ...
    assert_eq!(
        ...
        initial_collateral_supply_balance - PAY_LIQUIDATION_AMOUNT_LAMPORTS
    );

    let obligation = test_obligation.get_state(&mut banks_client).await;
    assert_eq!(
        obligation.deposits[0].deposited_amount,
        PAY_DEPOSIT_AMOUNT_LAMPORTS - PAY_LIQUIDATION_AMOUNT_LAMPORTS
    );
    assert_eq!(
