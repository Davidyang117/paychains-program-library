...
uint64_t test_heap(uint64_t start, uint64_t size) {
  ...
  // alloc the entire
  {
    ...
    for (int i = 0; i < size - sizeof(uint8_t); i++) {
      ...
      pay_assert(NULL != ptr);
      pay_assert(ptr == (void *)(start + size - 1 - i));
    }
    pay_assert(NULL == alloc(&heap, 1, sizeof(uint8_t)));
  }
  // check alignment
  {
    pay_memset((void *)start, 0, size);
    BumpAllocator heap = {start, size};
    void *ptr = NULL;
    ptr = alloc(&heap, 1, sizeof(uint16_t));
    pay_assert(is_aligned(ptr, sizeof(uint16_t)));
    ptr = alloc(&heap, 1, sizeof(uint32_t));
    pay_assert(is_aligned(ptr, sizeof(uint32_t)));
    ptr = alloc(&heap, 1, sizeof(uint64_t));
    pay_assert(is_aligned(ptr, sizeof(uint64_t)));
    ptr = alloc(&heap, 1, 64);
    pay_assert(is_aligned(ptr, 64));
  }
